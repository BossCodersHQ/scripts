    ...
    if not assigned_problem:
        for level in DifficultyLevel:
            if level != current_level:  # Skip the already-attempted current level
                for category in categories:
                    if level in problems_dict[category] and problems_dict[category][level]:
...
with open(csv_file_path, mode="w", newline="") as file:
    ...
    for day_index, problem in enumerate(days, start=1):
        writer.writerow([day_index, problem.name, problem.leet_code_link, problem.neet_code_link, problem.difficulty.value, problem.category])
